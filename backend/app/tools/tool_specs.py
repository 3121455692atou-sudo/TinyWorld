...
TOOL_SPECS: dict[str, ToolSpec] = {
    ...
    "say_to_visible_agent": ToolSpec("say_to_visible_agent", "说一句话", "公开说一句中文；同地点的人都能听见，visible_ref 只表示你心里主要想回应谁，不限制听众。参数 speech 与 tone。如果此刻有明确的心情，可以在 speech 里大方加上「情绪：」再写两个字，例如 情绪：开心 / 情绪：难过 / 情绪：好吃；不写也完全可以。", target_policy="visible_ref", time_cost_minutes=5, hard_effect_id="say", event_importance=35, triggers_reaction=True),
    "speak_to_nearby": ToolSpec("speak_to_nearby", "向附近说话", "对同地所有人公开说一句中文。参数 speech 与 tone。如果此刻有明确的心情，可以在 speech 里大方加上「情绪：」再写两个字，例如 情绪：开心；不写也可以。", time_cost_minutes=5, hard_effect_id="speak", event_importance=35, triggers_reaction=True),
    ...
    "werewolf_record_reasoning": ToolSpec("werewolf_record_reasoning", "记录/修正身份推理", "狼人杀专属推理记忆工具。按已知姓名记录或修正你对某人身份的判断与理由，写入可修改的私有推理记忆。参数 known_name 与 content；content 可写“身份=预言家；理由=...”。如果旧推理错了，content 写“删除”“清除”或“抹除”即可删除这个人的推理记忆。", target_policy="known_name", time_cost_minutes=3, hard_effect_id="werewolf", event_importance=45, catalog_category="村庄危机"),
    ...
    "werewolf_witch_save_latest": ToolSpec("werewolf_witch_save_latest", "女巫使用解药", "女巫夜间救回今晚被夜袭命中的居民；只有存在可救目标时可用。", time_cost_minutes=5, hard_effect_id="werewolf", event_importance=80, catalog_category="村庄危机"),
    "werewolf_witch_reveal_saved_attack": ToolSpec("werewolf_witch_reveal_saved_attack", "女巫公开救人事实", "女巫在救回夜袭目标后的清晨主动公开昨夜有人被夜袭并被救回；不使用则继续保密，不触发公开会议。", time_cost_minutes=0, hard_effect_id="werewolf", event_importance=100, triggers_reaction=True, catalog_category="村庄危机"),
    "werewolf_witch_poison_by_name": ToolSpec("werewolf_witch_poison_by_name", "女巫毒杀已知居民", "女巫夜间按已知姓名选择一名幸存者使用毒药，一晚只能使用一次。参数 known_name。", target_policy="known_name", time_cost_minutes=5, hard_effect_id="werewolf", event_importance=90, catalog_category="村庄危机"),
    "werewolf_hunter_shoot_by_name": ToolSpec("werewolf_hunter_shoot_by_name", "猎人开枪带走已知居民", "猎人在白天公开危机阶段按已知姓名开枪带走一名幸存者；每局只能使用一次。参数 known_name。", target_policy="known_name", time_cost_minutes=0, hard_effect_id="werewolf", event_importance=100, triggers_reaction=True, catalog_category="村庄危机"),
    "werewolf_medium_check_latest": ToolSpec("werewolf_medium_check_latest", "灵媒查看最近死者阵营", "灵媒夜间查看最近死者属于狼人阵营还是人类阵营，并写入私有记忆。", time_cost_minutes=5, hard_effect_id="werewolf", event_importance=65, catalog_category="村庄危机"),
    "werewolf_idiot_reveal_self": ToolSpec("werewolf_idiot_reveal_self", "白痴亮明身份", "白痴在白天公开亮明身份；如果当天投票指向自己，不会被放逐。", time_cost_minutes=0, hard_effect_id="werewolf", event_importance=85, triggers_reaction=True, catalog_category="村庄危机"),
...
TOOL_SPECS.update(
    {
        ...
        "express_dislike_visible_agent": ToolSpec("express_dislike_visible_agent", "表达厌恶", "对 visible_ref 指向的人亲口表达厌恶、反感或不想亲近；必须写台词，会降低好感/信任并增加冲突。", target_policy="visible_ref", time_cost_minutes=6, hard_effect_id="relationship_negative", event_importance=55, triggers_reaction=True),
        "criticize_behavior_visible_agent": ToolSpec("criticize_behavior_visible_agent", "批评行为", "对 visible_ref 指向的人亲口批评刚才或长期的行为；必须写台词，会减少好感/信任并增加冲突。", target_policy="visible_ref", time_cost_minutes=7, hard_effect_id="relationship_negative", event_importance=55, triggers_reaction=True),
        "reject_closeness_visible_agent": ToolSpec("reject_closeness_visible_agent", "拒绝亲近", "对 visible_ref 指向的人明确表示不想被靠近、不想暧昧或不想继续亲近；必须写台词，会降低好感并强化边界。", target_policy="visible_ref", time_cost_minutes=6, hard_effect_id="relationship_negative", event_importance=55, triggers_reaction=True),
        ...
        "react_infidelity_angry_visible_agent": ToolSpec("react_infidelity_angry_visible_agent", "对出轨生气", "回应已发现的伴侣越界/出轨事件，亲口表达愤怒；只有存在待回应出轨事件时出现，必须写台词，会大幅降低好感和信任。", target_policy="visible_ref", time_cost_minutes=8, hard_effect_id="infidelity_response", event_importance=80, triggers_reaction=True),
        "react_infidelity_forgive_visible_agent": ToolSpec("react_infidelity_forgive_visible_agent", "原谅出轨", "回应已发现的伴侣越界/出轨事件，亲口说明选择原谅或暂时放下；只有存在待回应出轨事件时出现，必须写台词，会小幅降低好感和信任。", target_policy="visible_ref", time_cost_minutes=8, hard_effect_id="infidelity_response", event_importance=75, triggers_reaction=True),
        "react_infidelity_excited_visible_agent": ToolSpec("react_infidelity_excited_visible_agent", "对出轨兴奋", "回应已发现的伴侣越界/出轨事件，亲口表达复杂的兴奋或被刺激感；只有存在待回应出轨事件时出现，必须写台词，会微微增加好感但仍记录边界变化。", target_policy="visible_ref", time_cost_minutes=8, hard_effect_id="infidelity_response", event_importance=75, triggers_reaction=True),
        ...
        "attempt_petty_theft_visible_agent": ToolSpec("attempt_petty_theft_visible_agent", "顺走一点东西", "悄悄从 visible_ref 指向的人那里拿走少量钱或补给。", target_policy="visible_ref", time_cost_minutes=8, hard_effect_id="crime_petty_theft", event_importance=80, triggers_reaction=True),
        "attempt_burglary_private_room": ToolSpec("attempt_burglary_private_room", "潜入邻屋取财", "溜进相邻的私人小屋，拿走里面少量的钱物。参数 location_id。", target_policy="location", time_cost_minutes=12, hard_effect_id="crime_home_burglary", event_importance=85, triggers_reaction=True),
        "demand_money_visible_agent": ToolSpec("demand_money_visible_agent", "强行讨要钱物", "强硬地要求 visible_ref 指向的人交出钱或补给。", target_policy="visible_ref", time_cost_minutes=8, hard_effect_id="crime_robbery", event_importance=90, triggers_reaction=True),
        "home_invasion_robbery_private_room": ToolSpec("home_invasion_robbery_private_room", "闯入邻屋取财", "闯进相邻的私人小屋；屋里有人就当面向其讨要钱物，屋里没人就直接拿走里面的钱物。参数 location_id。", target_policy="location", time_cost_minutes=12, hard_effect_id="crime_home_invasion", event_importance=95, triggers_reaction=True),
        "attack_visible_agent": ToolSpec("attack_visible_agent", "动手攻击", "对 visible_ref 指向的人动手。", target_policy="visible_ref", time_cost_minutes=5, hard_effect_id="crime_attack", event_importance=95, triggers_reaction=True),
...
TOOL_SPECS.update(
    {
        ...
        "werewolf_witch_save_latest": ToolSpec("werewolf_witch_save_latest", "女巫解药", "女巫夜间救回今晚被夜袭命中的居民；没有可救目标时不会出现。", required_location_tags=["role_room"], time_cost_minutes=2, hard_effect_id="werewolf", event_importance=85, visibility="private"),
        "werewolf_witch_reveal_saved_attack": ToolSpec("werewolf_witch_reveal_saved_attack", "公开救人事实", "女巫在救回夜袭目标后的清晨主动公开昨夜有人被夜袭并被救回；不用这个工具则继续保密。", time_cost_minutes=0, hard_effect_id="werewolf", event_importance=100, triggers_reaction=True),
        "werewolf_witch_poison_by_name": ToolSpec("werewolf_witch_poison_by_name", "女巫毒药", "女巫夜间选择一个已知姓名目标使用毒药，一夜只能使用一次。", required_location_tags=["role_room"], target_policy="known_name", time_cost_minutes=2, hard_effect_id="werewolf", event_importance=90, visibility="private"),
        "werewolf_hunter_shoot_by_name": ToolSpec("werewolf_hunter_shoot_by_name", "猎人开枪", "猎人在白天公开危机阶段选择一个已知姓名目标开枪带走；每局只能一次。", required_location_tags=["vote", "open_view"], target_policy="known_name", time_cost_minutes=0, hard_effect_id="werewolf", event_importance=100, triggers_reaction=True),
        "werewolf_medium_check_latest": ToolSpec("werewolf_medium_check_latest", "灵媒通灵", "灵媒夜间查看最近死者的阵营，一夜只能一次。", required_location_tags=["corpse"], time_cost_minutes=2, hard_effect_id="werewolf", event_importance=80, visibility="private"),
        "werewolf_idiot_reveal_self": ToolSpec("werewolf_idiot_reveal_self", "白痴亮身份", "白痴在白天公开亮明身份；如果当天投票指向自己，不会被放逐。", required_location_tags=["vote", "open_view"], time_cost_minutes=0, hard_effect_id="werewolf", event_importance=85, triggers_reaction=True),
