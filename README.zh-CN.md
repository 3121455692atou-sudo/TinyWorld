...
- Docker Compose 模式：使用 `docker/nas/` 目录里的 compose 文件在本机、服务器或 NAS 上从源码构建镜像。
...
仓库内置了一个本地构建用的 Docker Compose 项目，位置是 `docker/nas/`。它会在构建镜像时编译前端，并由后端在同一个端口提供网页和 API。
...
```text
http://服务器或NAS的IP:5174/
```

`.env` 里可以改端口和默认模型接口，例如 `AIWORLD_PORT`、`TLW_LLM_BASE_URL`、`TLW_LLM_API_KEY`、`TLW_WORLD_MODEL`、`TLW_PRO_MODEL`。不要把 `.env`、`config/`、`data/`、`logs/` 提交或公开。
